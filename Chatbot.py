...
try:
    with open("model_files\FAQindents.pickle", "rb") as f:
    ...
    with open("model_files\FAQindents.pickle", "wb") as f:
...
net = tflearn.fully_connected(net, 45)
net = tflearn.fully_connected(net, 45)
...
try:
    model.load("model_files\chatbot.tflearn")
except:
    model = tflearn.DNN(net)
    model.fit(training, output, n_epoch=1515, batch_size=8, show_metric=True)
    model.save("model_files\chatbot.tflearn")
...
def chat(inp):
    ...
    while True:
        inp_text = inp
        if(inp_text.lower() == "quit"):
            break
        results = model.predict([bag_of_words(inp_text, words)])
        result_index = np.argmax(results)
        pred_label = labels[result_index]
        print(results[0, result_index], pred_label)
        if(results[0, result_index] > 0.40):
            for tg in data["intents"]:
                if tg['tag'] == pred_label:
                    response = tg['responses']
            return random.choice(response)
        else:
            return str("I don't quite understand, try rephrasing your question")
